...
contract Strategy is IStrategy {
    ...
    function executeBuy(address token, uint256 amount) external payable override returns (uint256) {
        ...
        (uint256 valueToSend, SendParam memory sendParam, MessagingFee memory messagingFee) = prepareTakeTaxiAndAMMSwap(
            stargateRouter,
            dstChainId,
            amount,
            composer,
        ...
        stargate.send{ value: valueToSend }(sendParam, messagingFee, msg.sender);
    ...
    function prepareTakeTaxiAndAMMSwap(
        ...
        // FIXED: Always include lzReceive gas, and add compose gas if there's a compose message
        bytes memory extraOptions;

        if (_composeMsg.length > 0) {
            // Include both lzReceive gas AND compose gas
            extraOptions = OptionsBuilder.newOptions().addExecutorLzReceiveOption(200000, 0).addExecutorLzComposeOption(
                0,
                500000,
                0
            );
        } else {
            // Only include lzReceive gas
            extraOptions = OptionsBuilder.newOptions().addExecutorLzReceiveOption(200000, 0); // REQUIRED: Gas for lzReceive
        }
